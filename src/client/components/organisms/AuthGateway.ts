...
export function createAuthGateway({ auth }: AuthGatewayProps): NexusComponent {
  ...
  container.className =
    "nexus-flex nexus-flex-col nexus-gap-3 nexus-w-full nexus-px-6 nexus-py-2 nexus-pb-10 nexus-animate-fade-in";
  ...
  const buildProviderNode = (profile: ProviderProfile, isSkip = false) => {
    ...
    const providerKey =
      profile.provider === NexusProvider.GEMINI_API
        ? "gemini"
        : profile.provider === NexusProvider.COPILOT_PAT
          ? "github"
          : null;
    const isVerified = providerKey
      ? localStorage.getItem(`nexus_verified_${providerKey}`) === "true"
      : false;
      ...
      onVerify: async (type, value) => {
        ...
        try {
          ...
          if (res.ok && data.success) {
            ...
            textBody
              .querySelector(".nexus-text-slate-800")
              ?.classList.add("nexus-text-emerald-900");
            ...
            btn.classList.add(
              "nexus-bg-emerald-100",
              "nexus-text-emerald-700",
              "nexus-border-emerald-200"
            );
            import("../molecules/Toast").then(({ Toast }) =>
              Toast.show("SUCCESS_LINK_ESTABLISHED", "success")
            );
          } else {
            btn.innerHTML = `驗證失敗`;
            import("../molecules/Toast").then(({ Toast }) =>
              Toast.show(data.error || "INVALID_CREDENTIALS", "error")
            );
          }
        } catch {
          btn.innerHTML = `連線異常`;
    ...
    card.querySelector(".nexus-flex.nexus-items-center")?.addEventListener("click", () => {
      ...
      selectionGrid
        .querySelectorAll(".nexus-auth-card-selected")
        .forEach((c) => c.classList.remove("nexus-auth-card-selected"));
      selectionGrid
        .querySelectorAll(".nexus-auth-input-area")
        .forEach((a) => a.classList.add("nexus-hidden"));
      selectionGrid
        .querySelectorAll('.nexus-auth-card svg[style*="rotate(90deg)"]')
        .forEach((s) => ((s as HTMLElement).style.transform = ""));
      ...
      if (!isClosing) {
        ...
        import("@services/molecules/global-state").then(({ NexusStateStore }) =>
          NexusStateStore.setProvider(profile.provider)
        );
      } else {
        import("@services/molecules/global-state").then(({ NexusStateStore }) =>
  ...
  const resetBtn = createButton({
    ...
    icon: "trash-2",
    className:
      "nexus-w-full nexus-mt-3 nexus-bg-white nexus-text-slate-400 nexus-border-slate-100 nexus-hover-text-red-500 nexus-hover-bg-red-50 nexus-transition-all nexus-opacity-50 nexus-hover-opacity-100",
  });
  ...
  resetBtn.onclick = () => {
    const keys = [
      "nexus_gemini_key",
      "nexus_github_pat",
      "nexus_verified_gemini",
      "nexus_verified_github",
    ];
