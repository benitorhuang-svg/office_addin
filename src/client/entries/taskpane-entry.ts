...
// Register Fluent UI Web Components (tree-shaken — only what we import gets bundled)
import {
...
class TaskpaneLauncher {
  ...
  public async boot() {
    ...
    SocketService.on(SocketEvent.SYSTEM_STATE_UPDATED, (data: { power?: NexusPowerState; provider?: NexusProvider; isStreaming?: boolean; tokensPerSec?: number; ttft?: number; activePersona?: string }) => {
        ...
        if (data.tokensPerSec !== undefined) NexusStateStore.update({ tokensPerSec: data.tokensPerSec });
        if (data.ttft !== undefined) NexusStateStore.update({ ttft: data.ttft });
        if (data.activePersona !== undefined) NexusStateStore.update({ activePersona: data.activePersona });
    });
