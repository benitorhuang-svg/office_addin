...
export async function insertTextIntoExcel(text: string): Promise<void> {
  ...
  const grid: string[][] = lines.map((line) =>
    maxCount >= 1 ? line.split(bestDelimiter).map((cell) => cell.trim()) : [line.trim()]
  );
  ...
  try {
    ...
    console.warn(
      "[Excel Factory V10] Table mapping failed. Falling back to single-cell insertion.",
      getExcelErrorMessage(error)
    );
    await Excel.run(async (context) => {
      const selection = context.workbook.getSelectedRange();
...
export async function applyExcelActions(
          ...
          case "insert_heading":
            if (action.text) {
              ...
              if (action.type === "insert_heading") {
                ...
                sheet.getCell(currentRow, currentCol).format.font.size =
                  action.level === 1 ? 16 : 14;
              }
              currentRow++;
            }
            break;
...
export async function createExcelChart(
  ...
  return Excel.run(async (context) => {
    ...
    chart.width = 450;
    chart.height = 300;
    chart.top = 20 + index * 320;
    chart.left = 500;

    console.log(
      `%c[Excel Factory] Multi-Chart Stack V16: Component [${index}] Applied to '${cleanTitle}'`,
