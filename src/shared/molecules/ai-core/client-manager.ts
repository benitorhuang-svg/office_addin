...
import { cleanupAllSessions as cleanupAllSess } from "@shared/molecules/ai-core/session-lifecycle.js";
...
class ClientManager {
   ...
   */
  public static async warmUp(method: ACPConnectionMethod): Promise<void> {
    try {
      logger.info('ClientManager', 'Warming up client', { method });
      // We pass empty options or default options depending on method
      // For Copilot CLI and Gemini CLI, they usually don't need options for start()
      await this.getClient(method, {} as CopilotClientOptions);
    } catch (error) {
...
export const warmUpClient = ClientManager.warmUp.bind(ClientManager);
export const cleanupAllSessions = cleanupAllSess;
