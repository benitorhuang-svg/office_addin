import type { AgentEnvironment, ACPConnectionMethod } from '@shared/atoms/ai-core/types.js';
...
export function describeACPMethod(method: ACPConnectionMethod): { label: string; description: string } {
  const descriptions: Record<ACPConnectionMethod, { label: string; description: string }> = {
    gemini_cli: {
      ...
      description: '透過 Gemini CLI 的 --acp 模式串接，支持任何 ACP 兼容 Agent',
    },
    copilot_cli: {
      label: 'GitHub Copilot CLI',
      description: '使用現有 GitHub Copilot 訂閱權限，預設串接方式',
    },
    azure_byok: {
      label: 'Azure OpenAI (BYOK)',
      description: '使用自己的 Azure OpenAI 金鑰，支持自訂模型部署',
    },
    remote_cli: {
      label: '遠端 Copilot CLI',
      description: '連接到遠端運行的 Copilot CLI 實例 (copilot --acp --port)',
    },
  };

  return descriptions[method] || { label: method, description: '未知的連接方式' };
}
