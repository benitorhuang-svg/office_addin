...
export async function selfCorrect(
  ...
  const { domain, traceId, reviewer = reviewDesign, maxRetries = 2, refinementConstraints = [] } = opts;
  ...
  log.info(TAG, `First-pass review complete`, { domain, score: firstScore, passed: firstScore >= threshold });

  if (firstScore >= threshold) {
    return { content: currentContent, review: currentReview, healed: false, firstPassScore: firstScore };
  }
  ...
  while (currentReview.totalScore < threshold && attempts < maxRetries) {
    attempts++;
    log.warn(TAG, `Score ${currentReview.totalScore} < ${threshold} ??triggering correction pass ${attempts}/${maxRetries}`, { domain });

    const refinementPrompt = buildRefinementPrompt(prompt, currentReview, threshold, refinementConstraints);
  ...
  if (currentReview.totalScore < threshold) {
    log.warn(TAG, `Self-correction exhausted after ${attempts} attempts and still below quality threshold`, {
      score: currentReview.totalScore,
      threshold,
      domain,
    });
...
function buildRefinementPrompt(originalPrompt: string, review: DesignReviewResult, threshold: number, refinementConstraints: string[]): string {
  const issueList = review.allIssues.slice(0, 5).map((issue, i) => `  ${i + 1}. ${issue}`).join('\n');
