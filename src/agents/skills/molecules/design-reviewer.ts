...
function scoreVisualPoetry(output: string, domain: DesignDomain): DimensionScore {
  ...
  if (domain === 'ppt') {
    ...
    if (!/color|palette|hsl\(|rgb\(|#[0-9a-f]{3,6}/i.test(output)) {
      issues.push('No color specification found ??define a purposeful 2?? color palette.');
      score -= 6;
    }
...
function scoreUsabilityLegibility(output: string, domain: DesignDomain): DimensionScore {
  ...
  if (domain === 'ppt') {
    ...
    if (bulletLines > 6) {
      issues.push(`${bulletLines} bullet points detected ??reduce to ?? lines per slide for cognitive clarity.`);
      score -= 6;
    }
  }
  ...
  if (domain === 'word') {
    ...
    if (sentenceCount > 0 && output.length / sentenceCount > 220) {
      issues.push('Average sentence is very long ??aim for ??5 words per sentence for readability.');
      score -= 5;
    }
  }
