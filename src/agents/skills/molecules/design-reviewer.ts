...
function scoreInformationArchitecture(
  ...
  if (domain === "excel") {
    if (!/header|column|table|row 1/i.test(safeOutput)) {
      issues.push(
        "No clear header row definition detected — spreadsheets need a labelled header row."
      );
      score -= 10;
    }
    ...
    if (safeOutput.split("\n").filter((l) => l.trim()).length < 5) {
      issues.push(
        "Document is too brief — a structured document should have at least 5 non-empty paragraphs."
      );
      score -= 8;
    }
  }
...
function scoreVisualPoetry(
  ...
  if (domain === "excel") {
    ...
    if (!/bold|italic|fill|background|highlight|conditional.?format/i.test(output)) {
      issues.push(
        "No cell formatting specified — use header bold/fill and conditional formatting for visual clarity."
      );
      score -= 8;
    }
    if (!/align|center|left|right/i.test(output)) {
      issues.push(
        "Column alignment not addressed — numbers should be right-aligned, text left-aligned."
      );
      score -= 6;
    }
  } else if (domain === "ppt") {
    ...
    if (!/bold|italic|emphasis|highlight/i.test(output)) {
      issues.push(
        "No typographic emphasis — use bold/italic to create visual rhythm in body text."
      );
      score -= 6;
    }
  }
...
function scoreEmotionalResonance(
  ...
  if (domain === "excel") {
    ...
    if (!/because|so that|which allows|this helps|result/i.test(safeOutput)) {
      issues.push(
        "Output lacks rationale — explain why each formula or operation is used, not just the syntax."
      );
      score -= 8;
    }
    if (wordCount < 40) {
    ...
    if (wordCount < 80) {
      issues.push(
        "Slide content is too sparse — include supporting context that builds the audience's journey."
      );
      score -= 8;
    }
    ...
    if (!/next steps|conclusion|summary|call to action|cta/i.test(safeOutput)) {
      issues.push(
        'Missing "Call to Action" or "Next Steps" — slides should drive a clear objective.'
      );
      score -= 5;
    }
  } else if (domain === "word") {
    if (wordCount < 100) {
      issues.push(
        "Document text is extremely short — Word documents should provide comprehensive coverage."
      );
      score -= 5;
    }
  }
...
function scoreUsabilityLegibility(
  output: string | undefined,
  domain: DesignDomain,
  ...
  if (domain === "excel") {
    ...
    if (/=\w+\(/.test(safeOutput) && !/\$[A-Z]\$\d|\$[A-Z]\d|[A-Z]\$\d/g.test(safeOutput)) {
      issues.push(
        "Formulas present but no absolute references ($) detected — verify anchoring is intentional."
      );
      score -= 5;
    }
    if (!/explain|note|comment|\/\//i.test(safeOutput)) {
      issues.push(
        "Missing explanatory notes for complex operations — add comments for maintainability."
      );
      score -= 7;
    }
    // Check for data validation mention on input cells
    if (
      /input|entry|user.?fill/i.test(safeOutput) &&
      !/validation|restrict|dropdown/i.test(safeOutput)
    ) {
      issues.push(
        "Input cells detected without data validation — add dropdowns or restrictions to prevent errors."
      );
      score -= 8;
    }
  } else if (domain === "ppt") {
    ...
    for (const m of fontMatches) {
      ...
      if (size > 0 && size < 18) {
        issues.push(
          `Font size ${m} is below 18pt — violates WCAG readability for projected slides.`
        );
        score -= 8;
        break;
      }
    ...
    if (!isAgenda && bulletLines > 6) {
      issues.push(
        `${bulletLines} bullet points detected — reduce to ≤6 lines per slide for cognitive clarity.`
      );
      score -= 6;
    }
  } else if (domain === "word") {
    ...
    if (sentenceCount > 0) {
      ...
      if (avgLen > threshold) {
        issues.push(
          `Average sentence is too long (${Math.round(avgLen)} chars) — aim for shorter sentences for readability.`
        );
        score -= 5;
      }
    }
...
function scoreBrandConsistency(
  ...
  if (domain === "excel") {
    ...
    if (colorCount > 4) {
      issues.push(
        `${colorCount} distinct colors found — constrain to a 3-color scheme (header, data, highlight) for brand consistency.`
      );
      score -= 8;
    }
    if (!/number.?format|currency|percentage|decimal/i.test(safeOutput)) {
      issues.push(
        "No number format specified — define consistent formats (currency, percentage, etc.) for all numeric columns."
      );
      score -= 7;
    }
  } else if (domain === "word") {
    ...
    if (
      !/preferred term|glossary|terminology|consistently/i.test(safeOutput) &&
      safeOutput.length > 500
    ) {
      issues.push(
        "No mention of terminology consistency — verify output aligns with provided Glossary."
      );
      score -= 5;
    }
    const colorCount = new Set(
    ...
    if (colorCount > 6) {
      issues.push(
        `${colorCount} distinct color values — constrain to a 3-color palette for brand consistency.`
      );
      score -= 8;
    }
  } else {
    ...
    if (fontFamilyMatches > 2) {
      issues.push(
        `${fontFamilyMatches} typefaces found — limit to 2 (one sans-serif headline, one body).`
      );
      score -= 7;
    }
  }
...
export function reviewDesign(output: string, domain: DesignDomain): DesignReviewResult {
  const safeOutput = output ?? "";
  const isOfficeJs = /function|Excel\.run|Word\.run|PowerPoint\.run|context\.sync\(\)/i.test(
    safeOutput
  );
  ...
  const worstDimension = [...dimensions].sort(
    (a, b) => a.score / a.maxScore - b.score / b.maxScore
  )[0];
  const refinementHint =
    allIssues.length > 0 && worstDimension
