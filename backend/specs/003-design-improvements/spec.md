...
| 層 | 檔案 | 職責 |
|---|---|---|
| 入口 | `backend/server.ts` | Express server 啟動 |
| 技能路由 | `backend/skills/skill-orchestrator.ts` | 關鍵字 if/else 路由到 excel/ppt/word/shared |
| 技能定義 | `backend/skills/skills-manifest.json` | 技能觸發條件與引擎對應 |
| 連線池 | `backend/services/copilot/molecules/client-manager.ts` | Copilot SDK 連線管理 |
| 本地 agent | `backend/local-agent/index.mjs` | 獨立 agent 入口 |
| Shared skills | `backend/skills/shared/` | VectorNexus / GalaxyGraph / VisionExpert 等 |
