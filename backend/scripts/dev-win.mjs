...
// ─── Skill Bridge (FastAPI — eliminates Python cold start) ─────
const skillBridgePort = process.env.SKILL_BRIDGE_PORT || '8765';
const skillsBridgePath = path.join(repoRoot, 'backend', 'skills', 'skill_bridge.py');
const python = process.platform === 'win32' ? 'python' : 'python3';
startProcess(python, ['-m', 'uvicorn', 'skill_bridge:app', '--host', '127.0.0.1', '--port', skillBridgePort, '--app-dir', path.join(repoRoot, 'backend', 'skills')], 'skill-bridge');
