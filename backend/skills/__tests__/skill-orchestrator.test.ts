...
jest.mock('../shared/shared-invoker.js', () => ({
  SharedSkillInvoker: {
    invokeGalaxyGraph: jest.fn().mockImplementation(() => Promise.resolve({ status: 'galaxy_complete' })),
    invokeVectorSearch: jest.fn().mockImplementation(() => Promise.resolve({ status: 'vector_complete', results: [] })),
    getOmniBridgePromptPath: jest.fn().mockReturnValue('/fake/omni.md'),
  },
}));
jest.mock('fs', () => ({
  promises: {
    readFile: jest.fn().mockImplementation(() => Promise.resolve('# Mock Prompt Content\n\nThis is a test prompt.')),
  },
}));
jest.mock('../../core/atoms/logger.js', () => ({
  logger: {
...
import { classifyIntent, type IntentLabel } from '../atoms/intent-classifier.js';
...
describe('SkillOrchestrator', () => {
  ...
  describe('brand intelligence shortcut', () => {
    it('intercepts "brand: https://..." before intent classification', async () => {
      mockExtractBrandTokens.mockResolvedValue({
        ok: true,
        tokens: {
          primary: '#1a1a1a', secondary: '#ffffff', accent: '#3b82f6',
          background: '#ffffff', text: '#111827', cssBlock: '', sourceUrl: 'https://example.com',
        },
  ...
  describe('intent routing', () => {
    it('routes "ppt" intent and returns designReview metadata', async () => {
      ...
      const result = await orchestrator.route('create a presentation', baseCtx) as Record<string, unknown>;
    ...
    it('routes "word" intent with designReview', async () => {
      ...
      const result = await orchestrator.route('write a report', baseCtx) as Record<string, unknown>;
    ...
    it('routes "excel" intent without designReview', async () => {
      ...
      const result = await orchestrator.route('analyze spreadsheet', baseCtx) as Record<string, unknown>;
    ...
    it('routes "galaxy_graph" to SharedSkillInvoker', async () => {
      ...
      const result = await orchestrator.route('show dependency graph', { ...baseCtx, repo: 'owner/repo' });
    ...
    it('returns default vector_search for unknown intent', async () => {
      mockClassifyIntent.mockResolvedValue('vector_search' as IntentLabel);
      const result = await orchestrator.route('general question', baseCtx) as Record<string, unknown>;
