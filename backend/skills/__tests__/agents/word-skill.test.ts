 ...
 *  - ok:true with data + meta
 *  - ok:false on bridge error
 *  - changes array forwarded as edits via invoker
 *  - traceId propagation
 */
...
import { wordSkill } from "../../agents/word-skill.js";
...
describe("WordSkill (agent interface)", () => {
  ...
  it("stringifies non-Error throws", async () => {
    mockInvoke.mockRejectedValue({ code: 500 });

    const result = await wordSkill.execute({
      output_path: "/tmp/fail.docx",
      changes: [],
    });
  ...
  it("works without context (context is optional)", async () => {
    ...
    await expect(
      wordSkill.execute({ output_path: "/tmp/no-ctx.docx", changes: [] }),
