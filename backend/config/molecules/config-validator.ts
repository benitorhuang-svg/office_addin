  ...
  .superRefine((env, ctx) => {
    ...
    if (!hasGithub && !hasGemini && !hasAzure) {
      ctx.addIssue({
        ...
        message:
          '缺少必要的 AI 認證：需提供 GITHUB_PAT_TOKEN、GEMINI_API_KEY，或完整的 Azure OpenAI 憑證 (API_KEY + ENDPOINT + DEPLOYMENT)。',
      });
    }
  });
...
export const ConfigValidator = {
  validate() {
    ...
    if (!result.success) {
      ...
      throw new Error(
        `[ConfigValidationError] 基於安全與穩定性考量，伺服器無法啟動。請檢查 .env 設定。\n${issues}`
      );
    }
